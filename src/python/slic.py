...
# from https://github.com/laixintao/slic-python-implementation

# a class for cluster of each superpixel
...
class SLICProcessor(object):
    ...
    # initialize clustering based on simple gird sample.
    ...
    def move_clusters(self):
        # only done once at the initialization stage.
        # move the clusters centers to the place with lower gradient
        for cluster in self.clusters:
            cluster_gradient = self.get_gradient(cluster.h, cluster.w)
            # test for the four possibilities for movemet
            for dh in range(-1, 2):
                for dw in range(-1, 2):
                    _h = cluster.h + dh
                    _w = cluster.w + dw
    ...
    def assignment(self):
        for cluster in self.clusters:
            # for pixels near each cluster calculate the distance to the cluster
            # if close assign the pixel to the cluster closest.
    ...
    def update_cluster(self):
        # recompute all the cluster centers
        # and update the cluster LAB as this center pixel's values.
        for cluster in self.clusters:
