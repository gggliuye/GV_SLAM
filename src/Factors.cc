...
bool ProjectionCameraMapFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    if(jacobians){
        ...
        if(jacobians[1]){
            ...
            jacobians[1][0] = - rotation_mat(0,0) * fx_invz + x_fx * r02_invz2;
            jacobians[1][1] = - rotation_mat(0,1) * fx_invz + x_fx * r12_invz2;
            jacobians[1][2] = - rotation_mat(0,2) * fx_invz + x_fx * r22_invz2;
            jacobians[1][3] = - rotation_mat(0,1) * fy_invz + y_fy * r02_invz2;
            jacobians[1][4] = - rotation_mat(1,1) * fy_invz + y_fy * r12_invz2;
            jacobians[1][5] = - rotation_mat(1,2) * fy_invz + y_fy * r22_invz2;
        }
    }
    return true;
}
